...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if ($existingClip) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $revealClipUploadResult = $existingClip->handleVideoUpload('reveal_image', '../uploads/clips/');
            if ($revealClipUploadResult['success']) {
                $existingClip->setRevelClipUrl($revealClipUploadResult['file_path']); // Store file path instead of URL
                ...
                $_SESSION['error'] = $revealClipUploadResult['error_message'];
            }
        }
    } else {
